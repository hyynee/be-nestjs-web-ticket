...
const shouldRunPaymentIntegration =
  process.env.RUN_PAYMENT_INTEGRATION === "true";
const describePaymentIntegration = shouldRunPaymentIntegration
  ? describe
  : describe.skip;
...
beforeAll(async () => {
  if (!shouldRunPaymentIntegration) {
    return;
  }
...
describePaymentIntegration(
  "PAY-001 — Webhook idempotency: MongoDB fallback khi Redis DOWN",
  () => {
...
describePaymentIntegration(
  "PAY-003 — PayPal atomicity: PendingConfirmation record khi DB write thất bại",
  () => {
    it("should write PendingConfirmation vào MongoDB TRƯỚC KHI processPaypalPayment, ngay cả khi DB transaction sau đó fail", async () => {
      ...
      expect(updatedPayment!.metadata.captureStatus).toBe(
        "PendingConfirmation"
      );
...
describePaymentIntegration(
  "PAY-005 — Partial refund: refundHistory cập nhật đúng (PAY-005 guard fix)",
  () => {
