/* eslint-disable @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-floating-promises, no-useless-catch */
import {
  Injectable,
  BadRequestException,
  NotFoundException,
...
export class BookingService {
  private readonly BOOKINGS_CACHE_LIST_KEY = new Set<string>(); 
  private readonly USER_BOOKINGS_CACHE_KEY = new Set<string>(); 
  constructor(
    ...
    @Inject(QueueService) private readonly queueService: import("@src/queue/queue.service").QueueService
  ) {}
  ...
  async createBooking(userId: string, data: CreateBookingDto) {
    ...
    try {
      ...
      // Offload booking confirmation email to queue
      if (result?.data) {
  ...
  async getMyBookings(
    ...
    const cachedData =
      await this.cacheManager.get<PaginatedResponse<Booking>>(cacheKey);
    if (cachedData) {
      return cachedData;
    }
    const filter: any = {
    ...
    try {
      await session.withTransaction(async () => {
        ...
        if (
          ...
          throw new BadRequestException(
            "Only unpaid pending bookings can be cancelled. Paid bookings must go through refund flow."
          );
        }
    ...
    try {
      return await session.withTransaction(async () => {
        ...
        for (const b of expiredBookings) {
          ...
          changedZoneIds.add(key);
        }

        await this.zoneModel.bulkWrite(
          // PMbulkWrite cho phép gửi nhiều write operations trong 1 request đến MongoDB thay vì gửi từng cái một (nếu có 100 booking expired thuộc 10 zone khác nhau thì thay vì 10 round-trips, chỉ cần 1)
