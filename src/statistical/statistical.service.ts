...
export class StatisticalService {
  ...
  // Example: cache heavy statistics
  async getCachedHotEventsByRevenue() {
    const cacheKey = 'stat:hotEventsByRevenue';
    const cached = await this.cacheManager.get<any>(cacheKey);
    if (cached) return cached;
    const data = await this.getHotEventsByRevenue();
    await this.cacheManager.set(cacheKey, data, 60);
    return data;
  }

  async getHotEventsByRevenue(): Promise<HotEventByRevenue[]> {
    const hotEvents = await this.paymentModel.aggregate<HotEventByRevenue>([
      {
        $match: {
          status: "succeeded",
          isDeleted: false,
  ...
  async getOverviewStatistics(
    ...
    const comparisonPaymentFilter: any = {
      status: "succeeded",
      isDeleted: false,
    };
    const comparisonTicketFilter: any = {
      isDeleted: false,
    ...
    const currentMonthRevenue =
      revenueComparisonResult[0]?.currentMonth?.[0]?.total || 0;
    const previousMonthRevenue =
      revenueComparisonResult[0]?.previousMonth?.[0]?.total || 0;
    ...
    const currentMonthTicketsSold =
      ticketComparisonResult[0]?.currentMonth?.[0]?.ticketsSold || 0;
    const previousMonthTicketsSold =
      ticketComparisonResult[0]?.previousMonth?.[0]?.ticketsSold || 0;
    const ticketsSoldDifference =
      currentMonthTicketsSold - previousMonthTicketsSold;
    ...
    const currentMonthCheckedIn =
      ticketComparisonResult[0]?.currentMonth?.[0]?.checkedIn || 0;
    const previousMonthCheckedIn =
      ticketComparisonResult[0]?.previousMonth?.[0]?.checkedIn || 0;
  ...
  async getTopSellingEvents(
    ...
    const data = [
      ...
      },
      {
        $sort: (by === "tickets"
          ? { ticketsSold: -1 }
          : { totalRevenue: -1 }) as Record<string, 1 | -1>,
      },
  ...
  async getTopPotentialCustomers(): Promise<TopPotentialCustomer[]> {
    const topCustomers =
      await this.bookingModel.aggregate<TopPotentialCustomer>([
        {
          $match: {
            paymentStatus: "paid",
            isDeleted: false,
  ...
  async getCheckInZones(eventId: string): Promise<CheckInZoneStatistics[]> {
    ...
    const checkInZones =
      await this.ticketModel.aggregate<CheckInZoneStatistics>([
        {
          $match: {
            eventId: new Types.ObjectId(eventId),
            isDeleted: false,
